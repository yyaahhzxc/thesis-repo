...
\begin{description}
    ...
    \item[Direct Normative Scope vs. Longitudinal/Computational Tax Construction.] The operational scope of the natural language inference engine is strictly delimited to direct normative and modal contradictions arising from explicit textual collisions (e.g., local prohibitions of federally authorized activities, unauthorized procedural preconditions, or explicit statutory penalty ceiling infractions). The system deliberately does not model multi-decade longitudinal legislative baselines, historical tax bracket compounding, or equitable judicial rate adjustments. As demonstrated in \textit{Mindanao Shopping Destination Corp. v. Duterte}~\cite{scp2017mindanaoshopping}, determining whether an ordinance's 1.25\% local business tax rate impermissibly exceeded statutory limits required the Supreme Court to reconstruct an unprovided 1990 baseline rate of 0.50\% and calculate 10\% compound step-ups across 15 years of uncodified legislative history, even though the nominal rate rested below the statutory ceiling of 1.50\% in Section~143(d) of Republic Act No.~7160. Modeling such longitudinal statutory construction and complex multi-hop financial arithmetic exceeds the ex-ante textual reasoning capabilities of cross-encoder architectures and is preserved as an avenue for specialized fiscal analytics.
\end{description}
