...
\subsection{Research Design}
    ...
    \subsection{Corpus Acquisition and Exploratory Analysis}
...
def extract_statutory_corpus(base_url, start_year=1900, end_year=2026):
    ...
    return statute_records
        ...
        \begin{enumerate}
            \item \textbf{Davao City SP Public Web Portal:} Provides selective historical ``Landmark Ordinances'' and ad-hoc municipal uploads spanning approximately fifteen years, capturing prominent regulatory codes in public safety, zoning, and health~\cite{spdavao2026}.
            \item \textbf{Legislative Information Support System Program (LISSP):} Acquired directly through formal administrative coordination with the Sangguniang Panlungsod Secretariat and Information Technology division, who officially granted the proponents access to the LISSP documents via a shared Google Drive repository containing digitized enactments primarily from the 19th and 20th City Councils between 2021 and 2025~\cite{citygov2025lissp}.
        \end{enumerate}
        Together, these two channels yielded an archive of exactly 1,664 municipal enactments spanning from 1950 to 2025. These 1,664 documents represent the complete accessible digital corpus gathered within the proponents' institutional capabilities. Crucially, the true cumulative historical total of ordinances enacted since Davao City was chartered under Commonwealth Act No.~51 in 1936 remains unrecorded on the internet and unknown to municipal authorities. In institutional consultations, Sangguniang Panlungsod personnel confirmed that an official cumulative ledger does not exist because local legislation is numbered sequentially per calendar year and council term rather than in an unbroken cumulative sequence. Furthermore, the vast majority of historical enactments, particularly those prior to the 2021 LISSP digital initiative, exist exclusively as physical paper documents preserved in archive rooms and filing cabinets at the Sangguniang Panlungsod building on San Pedro Street, requiring manual page-by-page physical inventorying to tally. When combined with the 25,432 national statutes, the total statutory search space comprises \textbf{27,096 legal enactments} ($N_{\text{sections}} = 176,421$).
        ...
        The municipal archive also shows an uneven time distribution and gaps in older records. For ordinances passed before 2021, city record-keeping mainly focused on digitizing major ``Landmark Ordinances,'' leaving ordinary regulations from earlier decades in physical paper storage~\cite{spdavao2026}. As a result, the pre-2021 local records represent a selective collection of prominent laws rather than a complete legislative record. While post-2021 records from the digital LISSP database are much more complete, some numerical gaps remain due to missing committee drafts and unindexed attachments. These temporal biases mean that the available municipal archive contains substantially more recent ordinances than older enactments.

        The physical state of the scanned city ordinances adds both visual and linguistic noise. Unlike national laws extracted from clean HTML repositories or digital-born text exported directly from word processors, the raw municipal records consist entirely of scanned physical paper PDFs. Older ordinances scanned at low resolution suffer from faded print, ink smudges, and crooked pages, as illustrated in Figure~\ref{fig:davao_ordinance_scan}. The paper copies also contain non-text items such as government stamps, official signatures, and handwritten notes that confuse sentence splitting tools. At the language level, local ordinances sometimes include regional terms and administrative Taglish in their introductory sections~\cite{dreisbach2021language}. These informal drafting habits differ noticeably from the formal, standardized English found in national laws.
        ...
        \subsubsection{Philippine National Statutory Corpus Analysis}
        ...
        A key methodological decision in compiling this database is the explicit inclusion of \textbf{Executive Orders (EOs)} alongside congressional legislation. Under Article~VII, Section~17 of the 1987 Philippine Constitution and Book~III of Executive Order No.~292~\cite{philgov1987eo292}, Executive Orders are presidential acts providing rules of a general character in execution of statutory mandates. Unlike Supreme Court decisions that settle private adversarial controversies under \textit{stare decisis}, Executive Orders operate as \textit{ex-ante}, forward-looking statutory instruments possessing the full force of law. Modern Executive Orders establish binding nationwide regulatory mandates, such as Executive Order No.~26 (s.~2017) mandating nationwide public smoking bans, that legally preempt inconsistent local ordinances. Excluding Executive Orders would create an artificial regulatory gap, allowing local ordinances to contradict operative presidential mandates.
        ...
        The topic classification pipeline groups documents automatically while executing efficiently on the Primary Ingestion Workstation. Statutory texts are first stripped of HTML tags and filtered against a curated list of 49 legal stopwords, such as ``hereby'' and ``enacted,'' which appear ubiquitously without distinguishing legal subjects. Cleaned texts are converted into term frequency-inverse document frequency (TF-IDF) feature vectors across a 25,000-term vocabulary, rewarding distinctive statutory terminology over frequent administrative vocabulary~\cite{manning2008introduction}. To maintain computational efficiency while preserving conceptual cohesion, the pipeline applies Truncated Singular Value Decomposition (SVD) to project the sparse feature vectors into 64 latent semantic dimensions~\cite{deerwester1990indexing}. Following the latent semantic modeling principles of Landauer et al.~\cite{landauer1998introduction}, this 64-dimensional subspace captures core semantic associations while eliminating lexical noise, followed by standard $L_2$ vector normalization.

        Partitioning statutory texts directly into a small set of broad categories risks conflating heterogeneous legislative subjects. To prevent premature cluster mixing, the normalized 64-dimensional vectors are initially partitioned into $K = 28$ micro-clusters using MiniBatch $K$-Means clustering~\cite{sculley2010webscale}. Following the over-clustering strategy recommended by Grootendorst~\cite{grootendorst2022bertopic} and Aggarwal~\cite{aggarwal2012mining}, this over-complete partition isolates narrow recurring enactments, such as franchise charters or school conversions, into tight homogenous groups. The pipeline then adapts class-based TF-IDF ($c\text{-}TF\text{-}IDF$) from Grootendorst~\cite{grootendorst2022bertopic} to identify salient keywords by treating all text within a micro-cluster as a unified document:
        \[
            c\text{-}TF\text{-}IDF_{i, c} = \frac{|t_{i, c}|}{|c|} \times \log\left(1 + \frac{A}{\sum_{k} |t_{i, k}|}\right)
        \]
        where $|t_{i, c}|$ denotes the frequency of term $i$ in cluster $c$, $|c|$ is the total word count of cluster $c$, and $A$ indicates the average word count per cluster across the archive. This cluster-level weighting elevates terminology that specifically characterizes a legislative group while penalizing ubiquitous statutory phrasing.

        To align mathematical clusters with operative jurisprudence, the extracted $c\text{-}TF\text{-}IDF$ profiles are mapped against regulatory competencies defined in the Local Government Code of 1991~\cite{philgov1991ra7160}. Micro-clusters sharing congruent regulatory functions are merged into consolidated macro domains, requiring each domain to contain at least 1.0\% of the corpus ($N \ge 254$ statutes) to ensure statistical stability~\cite{aggarwal2012mining}. Outlier micro-clusters falling below this volume threshold are reassigned to their nearest semantic centroid, producing balanced clusters suitable for coarse indexing:
        \[
        ...
        \]
        where $\mathbf{z}_i$ represents the normalized 64-dimensional SVD vector of document $i$, $\mathbf{C}_m$ denotes the normalized centroid of macro domain $m$, and $\tau = 6.0$ functions as the inverse temperature scaling parameter. Adapting temperature scaling principles from Guo et al.~\cite{guo2017calibration}, the parameter $\tau = 6.0$ prevents uncalibrated softmax dispersion across cosine distances, ensuring that dominant domain probabilities remain sharply separated. Executing this consolidation across all 25,432 national statutes organizes the historical collection into eight Consolidated Macro Legal Domains, as detailed in Table~\ref{tab:national_macro_domains} and illustrated in Figure~\ref{fig:macro_domain_distribution}.
        ...
        The supervised benchmark results confirm that the discovered macro domains generalize reliably to unseen legislation. In an eight-domain classification setting, the uniform proportional chance criterion established by Morrison~\cite{morrison1969interpretation} yields a default random baseline accuracy of only 12.50\% ($1/8$). In contrast, the validation classifier reached an out-of-sample accuracy of 97.37\% and a Macro F1-Score of 96.26\%, surpassing the 85.0\% target threshold typically expected for reliable document categorization~\cite{aggarwal2012mining}. To provide an external sanity check beyond learned cluster labels, the evaluation tested four explicit statutory title probes ($N=1,259$ total) where legislative intent is unmistakable from the statutory catchline. The model achieved 97.8\% precision on telecommunications franchises ($N=360$), 96.6\% on hospital bed capacity acts ($N=203$), and 92.2\% on school infrastructure laws ($N=645$).

        The probabilistic confidence metrics evaluate how decisively the model assigns unseen statutes to their respective legal domains. Across all 5,087 holdout statutes, the model achieved a Mean Test Cosine Confidence of 0.5508, exceeding the typical 0.4000 operational target and demonstrating that the top assigned category captured dominant probability mass. Furthermore, 56.97\% of all unseen statutes were classified with high certainty ($\ge 0.50$ probability), well above the majority target threshold of 50.0\%. The holdout evaluation yielded a Cosine Silhouette Score of 0.1602 and a Davies-Bouldin Index of 2.4496, well within the target range formulated by Davies and Bouldin~\cite{davies1979cluster}. These geometric measurements prove that the topic model establishes stable, mathematically separated legal boundaries before documents are indexed for coarse retrieval.

        \subsubsection{Davao City Municipal Legislative Corpus Analysis}
        \label{sec:municipal_corpus_analysis}
        To evaluate local legislative patterns, the proponents conducted an empirical census of all 1,664 digitized Davao City municipal ordinances. Unlike the national archive which spans over a century of centralized governance, the municipal collection documents municipal self-governance across four distinct legislative eras. As illustrated in Figure~\ref{fig:local_ordinance_temporal_and_typology}(a), the archive contains 50 foundational enactments from the Pre-LGC Era (1950--1991), 184 landmark regulatory codes from the Post-LGC Historical Era (1992--2015), and 145 enactments from the Pre-Pandemic Era (2016--2020). Statutory volume concentrates heavily within the modern digital LISSP database, contributing 1,285 enactments (77.2\%) between 2021 and 2025. This temporal surge reflects the establishment of standardized electronic archiving by the Sangguniang Panlungsod, providing complete records of contemporary municipal lawmaking.

        \begin{figure}[!htbp]
            \centering
            \includegraphics[width=0.96\linewidth]{figs/local_ordinance_temporal_and_typology.png}
            \caption{Temporal Distribution across Four Legislative Eras and Two-Dimensional Latent Semantic Topic Space of the Davao City Municipal Legislative Corpus ($N = 1,664$). Panel (a) illustrates digitized ordinance counts across historical eras, highlighting the digital archiving expansion between 2021 and 2025. Panel (b) depicts the two-dimensional latent semantic space generated through 64-component Truncated SVD, featuring $1.5\sigma$ covariance confidence ellipses and geometric cluster centroids ($\mathbf{C}_1\text{--}\mathbf{C}_6$) across the six discovered functional typologies.}
            \label{fig:local_ordinance_temporal_and_typology}
        \end{figure}

        Applying Truncated SVD to the municipal corpus revealed that city legislation divides into six functional archetypes grounded in the Local Government Code of 1991~\cite{philgov1991ra7160}. To eliminate procedural noise before modeling, the pipeline extracts substantive operative text while setting aside routine session attendance headers. Word vectors were constructed across an 8,000-feature vocabulary and projected into 64 latent semantic dimensions, capturing 50.65\% of the total vocabulary variance. Figure~\ref{fig:local_ordinance_temporal_and_typology}(b) displays the two-dimensional latent semantic topic space for the municipal collection. Emergency Quick Response Fund (QRF) calamity grants form an isolated, tight cluster in the upper-left quadrant ($\mathbf{C}_3$), while temporary road closures project into a compact traffic management cluster ($\mathbf{C}_5$). Inter-agency agreements ($\mathbf{C}_2$) and deeds of donation ($\mathbf{C}_4$) span an elongated horizontal subspace along the positive horizontal axis, corresponding to executive contract authorizations under Section~22(c) of Republic Act No.~7160. Direct police power enactments ($\mathbf{C}_1$) and zoning permits ($\mathbf{C}_6$) anchor the lower-left quadrant, governing local community conduct.

        A critical methodological comparison between the national statutory topic landscape (Figure~\ref{fig:semantic_topic_landscape_2d}) and the municipal semantic space (Figure~\ref{fig:local_ordinance_temporal_and_typology}(b)) is the degree of visual dispersion. In the national corpus, over 3,500 cookie-cutter school conversions and 2,000 public utility franchises collapse into sharp, isolated radial rays due to identical statutory drafting templates. In contrast, local municipal legislation is intrinsically cross-functional under Republic Act No.~7160. Under Sections 16, 21, 22(c), 455, and 458 of the Local Government Code, a single substantive regulatory ordinance routinely incorporates budgetary appropriations, mayoral contract-signing authorizations, and penal sanction clauses within the same enactment. Furthermore, local enactments share standardized council preambles (``WHEREAS'' recitals) and procedural session metadata. While projecting 64 latent dimensions onto two axes inevitably compresses non-linear variance and produces visual overlap, the full 64-dimensional space achieves an out-of-sample supervised classification accuracy of 86.79\% and a 0.7973 mean cosine confidence on holdout tests. Displaying the authentic principal dimensions alongside $1.5\sigma$ covariance density ellipses and centroids preserves empirical integrity without resorting to artificial dimensional cherry-picking. Table~\ref{tab:local_typology_composition} details the composition, percentage share, statutory authority, core subject matter, and top class-based TF-IDF keywords for each discovered archetype.
        ...
        \begin{table}[!htbp]
            ...
            \caption{Composition, Functional Typology, and Salient $c\text{-}TF\text{-}IDF$ Keywords of the Davao City Municipal Legislative Corpus ($N = 1,664$)}
            \label{tab:local_typology_composition}
            \begin{tabularx}{\linewidth}{c>{\RaggedRight}p{3.8cm}rr>{\RaggedRight}p{2.2cm}>{\RaggedRight}X}
                \toprule
                \textbf{ID} & \textbf{Functional Legislative Typology} & \textbf{Count} & \textbf{Share} & \textbf{Statutory Authority} & \textbf{Core Salient $c\text{-}TF\text{-}IDF$ Keywords} \\
                \midrule
                01 & Substantive Regulatory \& Penal & 684 & 41.11\% & RA 7160 \S458 & public, safety, fine, penal, violation, fee, health, traffic \\
                02 & Inter-Agency Agreements (MOA/MOU) & 481 & 28.91\% & RA 7160 \S\S22(c), 455 & pursuant, local, sign, panlungsod, contracts, agreement \\
                03 & Disaster Relief \& QRF Funds & 189 & 11.36\% & RA 10121 \S21 & fund, calamity, disaster, risk, reduction, quick response \\
                04 & Deeds of Donation \& Usufruct & 140 & 8.41\% & RA 7160 \S22(a)(4) & donation, local, powers, pursuant, contracts, deed, usufruct \\
                05 & Temporary Road Closures & 122 & 7.33\% & RA 7160 \S21 & road, closure, temporary, vehicular traffic, street, route \\
                06 & Zoning \& Subdivision Permits & 48 & 2.88\% & RA 7160 \S20, BP 220 & zoning, zone, land, reclassification, residential, commercial \\
                \midrule
                \multicolumn{2}{l}{\textbf{Total Municipal Collection}} & \textbf{1,664} & \textbf{100.00\%} & \multicolumn{2}{l}{\textbf{All Digitized Ordinances (1950--2025)}} \\
                \bottomrule
            \end{tabularx}
        \end{table}

        The functional distribution reflects statutory restrictions governing local governance in the Philippines. Substantive regulatory and penal enactments form the largest single group (41.11\%, $N=684$), containing community regulations such as truck bans, cemetery setbacks, stray animal controls, and drinking water safety. Meanwhile, administrative authorizations comprise nearly half of all council activity. Under Section~22(c) and Section~455(b)(1)(vi) of Republic Act No.~7160, the city mayor cannot bind the local government to contracts or accept donations without prior council approval. Consequently, inter-agency agreements (28.91\%), calamity fund authorizations under Republic Act No.~10121 (11.36\%), and deeds of donation (8.41\%) appear frequently in the municipal record.

        To verify that the discovered municipal typologies represent stable semantic boundaries, the proponents conducted holdout validation using an 80/20 stratified split ($N_{\text{train}} = 1,331, N_{\text{test}} = 333$). Table~\ref{tab:local_holdout_validation} summarizes the out-of-sample performance metrics, chance baselines, and explicit municipal title probes. The validation classifier achieved an out-of-sample accuracy of 86.79\% and a Weighted F1-Score of 84.84\%, far exceeding the 16.67\% uniform chance baseline ($1/6$). Predictions showed strong decisiveness, with a Mean Test Cosine Confidence of 0.7973 and 92.79\% of unseen ordinances classified with high confidence ($\ge 0.50$). Title probe verification on unambiguous indicator subsets confirmed high precision across Quick Response Fund releases (98.82\%, $N=169$), substantive penal regulations (96.10\%, $N=77$), temporary road closures (87.96\%, $N=108$), inter-agency agreements (86.56\%, $N=372$), and zoning permits (80.00\%, $N=15$). The lower precision on deeds of donation (29.77\%, $N=131$) occurs because donation acceptances are legally drafted as mayoral authorizations to sign deeds, directly sharing contractual syntax with inter-agency agreements under Section~22(c) of Republic Act No.~7160.
        ...
        \begin{table}[!htbp]
            ...
            \renewcommand{\arraystretch}{0.95}
            \setlength{\tabcolsep}{3.5pt}
            \caption{Empirical Holdout Validation Metrics, Baseline Comparisons, and Explicit Statutory Title Probes for the Municipal Topic Modeler ($N_{\text{train}} = 1,331, N_{\text{test}} = 333$)}
            \label{tab:local_holdout_validation}
            \begin{tabularx}{\linewidth}{>{\RaggedRight}p{4.6cm}cc>{\Centering}p{2.3cm}>{\RaggedRight}X}
                \toprule
                \textbf{Validation Metric / Benchmark Test} & \textbf{Observed} & \shortstack[c]{\textbf{Default}\\\textbf{Baseline}} & \shortstack[c]{\textbf{Target}\\\textbf{Range}} & \textbf{Empirical Verdict} \\
                \midrule
                Out-of-Sample Supervised Accuracy & 86.79\% & 16.67\% & $\ge 80.0\%$ & Strong Generalization \\
                Out-of-Sample Macro F1-Score & 75.39\% & 16.67\% & $\ge 70.0\%$ & Balanced Class Discrimination \\
                Out-of-Sample Weighted F1-Score & 84.84\% & 16.67\% & $\ge 80.0\%$ & High Volume Precision \\
                Mean Test Cosine Confidence & 0.7973 & 0.1667 & $> 0.5000$ & Dominant Probability Mass \\
                High-Confidence Predictions ($\ge 0.50$) & 92.79\% & $< 20.0\%$ & $> 75.0\%$ & Clear Majority Partition \\
                Holdout Cosine Silhouette Score & 0.0914 & $\le 0.0000$ & $0.0500\text{--}0.1500$ & Authentic Text Separation \\
                Holdout Davies-Bouldin Index & 3.7445 & $> 5.0000$ & $3.0000\text{--}4.0000$ & Compact Cluster Scatter \\
                \midrule
                \multicolumn{5}{l}{\textbf{Explicit Municipal Statutory Title Probes} (\textit{Unambiguous Subsets across Corpus, $N=872$})} \\
                \midrule
                Quick Response Fund / Calamity Assistance & 98.82\% & 16.67\% & $> 90.0\%$ & High Precision ($N=169$) \\
                Substantive Penal \& Police Power Regulations & 96.10\% & 16.67\% & $> 90.0\%$ & High Precision ($N=77$) \\
                Temporary Road Closure \& Traffic Diversions & 87.96\% & 16.67\% & $> 85.0\%$ & High Precision ($N=108$) \\
                Inter-Agency Accord / Authority to Sign MOA & 86.56\% & 16.67\% & $> 80.0\%$ & High Precision ($N=372$) \\
                Zoning \& Subdivision Development Permits & 80.00\% & 16.67\% & $> 75.0\%$ & High Precision ($N=15$) \\
                Deeds of Donation \& Property Usufruct & 29.77\% & 16.67\% & $> 25.0\%$ & Shared Mayoral Authorization ($N=131$) \\
                \bottomrule
            \end{tabularx}
        \end{table}

        Document lengths in the municipal collection differ noticeably from national statutes. Across all 1,664 local enactments, ordinances exhibit a median length of 5,644 characters and 880 words, with an interquartile range of 736 to 1,607 words. The municipal archive contains 11,801 total section provisions. Automated preprocessing isolates 8,160 operative substantive provisions (69.15\%) with a median length of 98 tokens, while pruning 3,641 procedural boilerplate sections (30.85\%) such as separability, repealing, and effectivity clauses.

        \subsubsection{Dual Statutory Corpus Integration and Comparative Analysis}
        \label{sec:dual_corpus_integration}
        Combining national legislation with Davao City municipal enactments establishes a unified search space of \textbf{27,096 legal records} comprising \textbf{176,421 searchable provisions}. In automated statutory conflict detection, evaluating draft measures requires examining both vertical statutory preemption and horizontal legislative consistency. Under the doctrine of statutory preemption articulated in \textit{Magtajas v. Pryce Properties}~\cite{scp1994magtajasvpryce}, local measures are tested against the 25,432 superior national statutes. Concurrently, under intra-jurisdictional coherence principles, draft measures are evaluated against the 1,664 existing city ordinances to prevent conflicting local commands, duplicate penalties, or unintended implied repeals. Table~\ref{tab:dual_corpus_comparison} summarizes the empirical parameters and comparative characteristics of the two statutory collections.
        ...
        \begin{table}[!htbp]
            ...
            \caption{Comparative Metrics and Structural Parameters of the Dual Statutory Search Space ($N = 27,096$ Enactments, $N_{\text{sections}} = 176,421$)}
            \label{tab:dual_corpus_comparison}
            \begin{tabularx}{\linewidth}{>{\RaggedRight}p{3.4cm}>{\Centering}p{1.7cm}>{\Centering}p{1.9cm}>{\Centering}p{1.8cm}>{\RaggedRight}X}
                \toprule
                \textbf{Empirical Dimension / Metric} & \textbf{National Corpus} & \textbf{Municipal Corpus} & \textbf{Combined Corpus} & \textbf{Analytical Significance} \\
                \midrule
                Total Enactments ($N$) & 25,432 & 1,664 & \textbf{27,096} & Comprehensive legislative space across jurisdictions \\
                Total Section Provisions & 164,620 & 11,801 & \textbf{176,421} & Total searchable provision-level retrieval units \\
                Operative Provisions & 164,620 & 8,160 & \textbf{172,780} & Candidate substantive rules evaluated for conflict \\
                Procedural Boilerplate Sections & 0 & 3,641 (30.85\%) & \textbf{3,641} & Filtered procedural clauses pruned before modeling \\
                Historical Enactment Span & 1900--2026 (126 yrs) & 1950--2025 (75 yrs) & \textbf{1900--2026} & Longitudinal statutory corpus across legal regimes \\
                Median Enactment Characters & 10,807 & 5,644 & \textbf{10,480} & Document-level length disparity across collections \\
                Median Enactment Words & 1,512 & 880 & \textbf{1,472} & Local enactments display concise regulatory scope \\
                Median Provision Tokens & 122.0 & 90.0 & \textbf{120.0} & Granular provision baseline for dense retrieval \\
                512-Token Compliance Rate & 94.83\% & 92.72\% & \textbf{94.69\%} & Standard transformer attention compatibility \\
                Operative 512-Token Compliance & 94.83\% & 90.06\% & \textbf{94.60\%} & High sequence compliance for operative text \\
                Normative Conflict Scope & Vertical Preemption & Horizontal Coherence & \textbf{Dual Scope} & Comprehensive statutory preemption and consistency \\
                \bottomrule
            \end{tabularx}
        \end{table}

        Figure~\ref{fig:dual_corpus_length_asymmetry} illustrates the empirical length disparity between the national and municipal collections. In Panel~(a), document-level character distributions on a logarithmic scale highlight the structural asymmetry between national codes and local ordinances. National statutes display a broad spread with an extreme right tail exceeding one hundred thousand characters, driven by omnibus codes such as the National Internal Revenue Code and Local Government Code. In contrast, municipal ordinances concentrate tightly around their median of 5,644 characters (880 words), reflecting their focused administrative nature. In Panel~(b), provision-level token density curves reveal that both national statutory sections (median: 122 tokens) and municipal operative provisions (median: 98 tokens) concentrate within 40 to 180 tokens. This distribution confirms that natural legislative sections are sufficiently granular to avoid semantic dilution while remaining well below the 512-token ceiling.

        \begin{figure}[!htbp]
            \centering
            \includegraphics[width=0.96\linewidth]{figs/dual_corpus_length_asymmetry.png}
            \caption{Enactment Length Asymmetry and Provision-Level Subword Token Densities across the Dual Statutory Search Space ($N = 27,096$ Enactments, $N_{\text{sections}} = 176,421$). Panel (a) compares enactment full-text character length distributions on a logarithmic scale, highlighting national omnibus codifications against concise municipal ordinances. Panel (b) depicts probability density curves for national statutory provisions (median: 122 tokens), municipal operative provisions (median: 98 tokens), and municipal boilerplate sections (median: 72 tokens) against the 512-token transformer context ceiling.}
            \label{fig:dual_corpus_length_asymmetry}
        \end{figure}

        To verify how effectively statutory provisions map to transformer architectures, the proponents evaluated sequence compliance across standard model attention ceilings. Figure~\ref{fig:dual_corpus_transformer_compliance}(a) traces the empirical cumulative distribution functions (CDF) across provision token lengths. Exactly 94.83\% of national provisions and 92.72\% of municipal provisions (90.06\% of operative substantive sections) fall strictly under the 512-token boundary. Across all 176,421 provisions in the combined search space, 94.69\% fit cleanly within standard BERT context windows without truncation. Figure~\ref{fig:dual_corpus_transformer_compliance}(b) compares compliance rates across candidate transformer context windows with dedicated legends for National Statutes and Davao City Local Ordinances. While compact 128-token windows accommodate only 53.8\% of national provisions, compliance increases to 82.3\% at 256 tokens and 94.8\% at 512 tokens, with long-context architectures such as ModernBERT capturing over 99.9\% of all provisions natively.

        \begin{figure}[!htbp]
            \centering
            \includegraphics[width=0.96\linewidth]{figs/dual_corpus_transformer_compliance.png}
            \caption{Cumulative Provision Compliance Curves and Attention Window Evaluation across the Dual Statutory Search Space ($N = 27,096$ Enactments, $N_{\text{sections}} = 176,421$). Panel (a) illustrates empirical cumulative distribution functions (CDF) across token lengths up to 1,024 tokens, denoting exact compliance at the 512-token threshold for national provisions (94.83\%), all municipal sections (92.72\%), and municipal operative provisions (90.06\%). Panel (b) provides a comparative grouped bar chart evaluating provision fit across five candidate transformer context windows (128, 256, 512, 1,024, and 8,192 tokens) with dedicated legends for National Statutes and Davao City Local Ordinances.}
            \label{fig:dual_corpus_transformer_compliance}
        \end{figure}

        To handle provisions in the 5.31\% exceedance tail without information loss under standard-context models, the pipeline implements a hierarchical overlapping sub-chunking fallback. Adapting the sliding window strategy formulated by Devlin et al.~\cite{devlin2019bert}, when an extracted section exceeds a safety threshold of 448 tokens, the preprocessing module divides the section into overlapping sub-chunks using a window of $w = 384$ tokens and a stride of $s = 256$ tokens (128-token overlap). Capping the sub-chunk window at 384 tokens reserves a 64-token structural buffer within the 512-token ceiling to accommodate the prepended metadata header along with mandatory special tokens ($[CLS]$, $[SEP]$)~\cite{devlin2019bert}. Each sub-chunk preserves the prepended structural metadata header $\left[ \text{Statute Title}, \text{ Section } S_i\text{: Catchline} \right]$ to maintain institutional context. In Stage~1 retrieval, the parent provision embedding is computed by mean-pooling sub-chunk vectors, capturing semantic concepts across all paragraphs. In Stage~2 inference, the Cross-Encoder evaluates each sub-chunk independently against the local ordinance hypothesis $h$, determining provision-level conflict through max-pooling across sub-chunk contradiction probabilities:
        \[
            \hat{P}(\text{Contradiction} \mid p_i, h) = \max_{j} P\left(\text{Contradiction} \mid p_{i,j}, h\right)
        \]
        where $p_{i,j}$ denotes the $j$-th sub-chunk of statutory section $p_i$. This max-pooling aggregation ensures that an isolated prohibition or restrictive condition buried deep in a multi-paragraph section triggers the contradiction alert without being diluted by surrounding agreeable text. Alongside the sub-chunking fallback, the proponents evaluate ModernBERT as a long-context candidate architecture~\cite{warner2024modernbert, kadowaki2025modernbert}, testing whether native 8,192-token attention processing outperforms standard-context models across lengthy codifications.

    \subsection{Preprocessing and Text Normalization Pipeline}
    \label{sec:preprocessing_filtering}
    Because raw statutory documents originate from disparate digital repositories and historical scanning archives, they require systematic ingestion, boundary segmentation, and text normalization. The preprocessing pipeline transforms heterogeneous legal text into standardized, context-enriched retrieval chunks optimized for coarse search and fine inference. This pipeline addresses four distinct technical requirements: digital-born draft ingestion, procedural boilerplate filtering, archival OCR error correction, and statutory exception handling. By systematically decoupling administrative boilerplate from operative commands, the pipeline ensures that downstream language models evaluate substantive legal mandates. The following subsections detail each operational stage of the text normalization architecture.

        \subsubsection{Digital-Born Draft Ingestion and Section Boundary Parsing}
        \label{sec:draft_ingestion_segmentation}
        A critical operational distinction in this research separates the historical training archive from live ex-ante draft submissions. While the historical Davao City collection consists of scanned physical PDFs requiring optical character recognition (OCR) and text normalization, newly drafted ordinances submitted for ex-ante review are digital-born documents. Legislative staff create these proposed measures using standard word processing software, generating clean electronic text or searchable PDF files. Because digital-born drafts contain authentic character encodings without scanning artifacts, the live pipeline bypasses OCR error correction entirely. This structural decoupling allows the preprocessor to focus immediately on automated section boundary parsing and clause segmentation.

        To evaluate legislative commands without semantic distortion, an automated regular expression parser splits the submitted draft at formal section boundaries into discrete operative blocks. In Philippine local legislative practice, proposed measures follow standardized structural templates established by the Department of the Interior and Local Government~\cite{dilg2022drafting}. Real-world legislative drafts contain substantial procedural boilerplate that does not express substantive legal commands. If routine clauses such as title declarations, repealing clauses, and effectivity dates are fed into Stage~1 retrieval, they match thousands of administrative national statutes containing similar formulaic phrases. Listing~\ref{lst:jsonl_sample} displays an authentic raw ingestion record from the national statutory collection, while Listing~\ref{lst:provision_extractor} details the regular expression parser that segments live draft measures into discrete operative sections while filtering out procedural boilerplate.
...
def extract_operative_provisions(draft_text):
    ...
    return operative_sections
        ...
        \subsubsection{Procedural Boilerplate Filtering and Structural Metadata Headers}
        \label{sec:boilerplate_and_metadata}
        Filtering formulaic boilerplate prevents search drift and computational waste during candidate retrieval. In both national statutes and municipal enactments, procedural clauses---specifically Title, Separability, Repealing, and Effectivity clauses---contain generic legislative phrases that carry zero substantive regulatory restrictions. When these sections are vectorized, their embeddings align with unrelated administrative acts that happen to share identical legal boilerplate. By systematically pruning these non-operative sections during ingestion, the preprocessor directs candidate retrieval exclusively toward substantive regulatory and penal provisions. As established in the corpus census, this filtering prunes 3,641 procedural sections (30.85\%) from the municipal collection, isolating 8,160 operative provisions for indexing.

        Once operative sections are isolated, evaluating granular statutory provisions requires preserving their governing legislative identity. An isolated sub-clause stripped of its parent enactment number and title suffers from severe semantic vector dilution, causing retrieval models to confuse municipal regulatory penalties with unrelated national tax or criminal codes. To maintain semantic precision and hierarchy across both stages, the pipeline applies structural context prepending. Listing~\ref{lst:context_prepending} details the context prepending protocol, while Listing~\ref{lst:model_ready_chunk} demonstrates the concrete transformation from an isolated provision into a Stage~1 indexed chunk and the resulting Stage~2 asymmetric input sequence. Every extracted statutory section begins with a structural metadata header containing the statute title, section number, and section title:
        \[
            \mathbf{x}_{\text{chunk}} = \left[ \text{Statute Title}, \text{ Section } S_i\text{: Catchline} \right] \mathbin{\Vert} \mathbf{t}_{\text{verbatim}}
        \]
        where $\mathbf{t}_{\text{verbatim}}$ denotes the exact text extracted from the statutory record, and $\mathbin{\Vert}$ represents string concatenation.
...
stage2_input_sequence = (
    ...
    "a fine of Fifteen Thousand Pesos (PHP 15,000.00) or two years imprisonment. [SEP]"
        ...
        The practical necessity for structural metadata headers is demonstrated by sections within composite legal codes. In Republic Act No.~7160, Section~447 sets penalty limits for municipalities, while Section~458 sets distinct penalty limits for city councils. Without adding the section title and number, downstream Cross-Encoders can confuse different levels of local government and misjudge municipal powers. Adding structural metadata ensures that the model evaluates each legal clause under its correct administrative level. As a result, the vector embeddings retain both the exact wording of the clause and its broader statutory context.

        \subsubsection{Archival OCR Normalization and Character Error Correction}
        \label{sec:ocr_normalization}
        While digital-born drafts contain clean electronic text, the historical Davao City municipal archive presents substantial optical character recognition (OCR) noise from digitized paper documents. Archival scans from earlier legislative eras frequently exhibit character substitutions, split words across line breaks, and stray non-alphanumeric symbols caused by document skew and low scanning resolution, as illustrated in Figure~\ref{fig:davao_ordinance_scan}. To prevent these scanning artifacts from degrading embedding fidelity during Stage~1 retrieval, the preprocessing module applies a targeted lexical normalization pipeline. The pipeline executes pattern-based corrections for common numerical and typographical OCR errors in ordinance headers, such as restoring malformed year sequences and standardized section catchlines. Hyphenated line-wrap breaks are reconciled against a curated Philippine statutory vocabulary dictionary, ensuring that legal terms are fully reconstructed before tokenization.

        \subsubsection{Harmonic Interpretation and Statutory Exception Handling}
        \label{sec:exception_handling}
        A key challenge in conflict detection is distinguishing genuine normative contradictions from authorized statutory exceptions. In regulatory legislation, general prohibitions and permitted exceptions frequently appear within the same section using conditional syntax. Because statutory sections average between 90 and 122 tokens, an entire section fits comfortably within the 512-token input limit of candidate Cross-Encoders. The model's attention mechanism reads the general prohibition and its qualifying exceptions simultaneously, preventing false positive contradiction alerts. This joint attention ensures that local measures implementing authorized statutory exceptions are recognized as valid exercises of delegated authority.

        When a general prohibition and an authorizing exception reside in separate statutory sections, the pipeline relies on two-stage candidate retrieval. Under Philippine statutory construction, related provisions governing the same subject matter must be interpreted together as a harmonious whole under the doctrine of \textit{in pari materia}~\cite{agpalo2009statutory}. If an ordinance draft relies on an authorized statutory exemption, evaluating a specific clause without retrieving the parent exception can trigger an incorrect conflict warning. The system addresses this challenge by utilizing hybrid retrieval in Stage~1 to pull both general bans and specific exception clauses into the candidate shortlist. The Stage~2 Cross-Encoder then evaluates the draft against the authorizing exception clause and assigns an Entailment classification, confirming that the local measure operates within valid legislative authority.
...
def detect_section_conflicts(draft_section, candidate_provisions, model, threshold=0.35, batch_size=16):
    ...
    \begin{description}
        ...
        \begin{itemize}
            ...
            \item \textbf{Local Taxation and Revenue Powers:} In \textit{City of Davao and Tanjili v. ARC Investors, Inc.}~\cite{scp2022davaovarc}, the Supreme Court voided Davao City's local business tax assessment under Ordinance No.~158-05, ruling that an LGU cannot unilaterally reclassify a passive holding company as a non-bank financial intermediary without Bangko Sentral ng Pilipinas authorization. In \textit{City of Davao v. First Meridian Development, Inc.}~\cite{scp2022firstmeridian}, the Court invalidated Section~423 of Davao City Ordinance No.~158-05 as ultra vires for imposing a mandatory ``payment under protest'' precondition on local business tax protests, holding that local legislative bodies cannot erect procedural barriers that restrict statutory remedies when Section~195 of Republic Act No.~7160 deliberately omits prior payment requirements for business tax appeals. In \textit{City of Davao v. Court of Appeals and GSIS}~\cite{scp2005davaovgsis}, the Court affirmed Davao City's municipal taxing authority, upholding real property tax assessments on GSIS commercial assets after establishing that Sections~193 and 234 of Republic Act No.~7160 effectively withdrew the prior statutory tax exemptions of government-owned and controlled corporations.
            \item \textbf{Public Health and Delegated Traffic Regulation:} Under the General Welfare Clause (Section~16 of Republic Act No.~7160), Davao City enacted landmark regulatory measures that established stricter local standards than national baselines without statutory invalidation, including the Comprehensive Anti-Smoking Ordinance (Ordinance No.~0367-12)~\cite{spdavao2012ord0367} establishing stricter setbacks than Republic Act No.~9211, and the Comprehensive Speed Limit Ordinance (Ordinance No.~0270-23)~\cite{spdavao2023ord0270} classifying local road speed ceilings pursuant to delegated authority under Republic Act No.~4136 and Joint JAO 2018-01.
        \end{itemize}

        Table~\ref{tab:tier3_cases} presents the complete Davao City municipal validation suite, detailing the local measures, governing national statutes, legal ratios decidendi, and gold NLI labels. To ensure long-term reproducibility and immediate system availability, complete archival copies of these eight Davao City ordinances and tax measures have been assembled and stored locally within the repository's municipal corpus (\texttt{corpus/davao\_city\_tier3\_ordinances/}). Standardized diagnostic problem-solution pairs are indexed in \texttt{data/tier3\_jurisprudential\_cases.jsonl}, pairing operative draft excerpts with corresponding statutory provisions and judicial holdings.
        ...
        \begin{table}[!htbp]
            ...
            \setlength{\tabcolsep}{2.5pt}
            \renewcommand{\arraystretch}{0.92}
            \caption{Landmark Davao City Municipal Jurisprudential Suite for Tier~3 System Evaluation ($N = 8$ Local Ordinances and Preemption Disputes)}
            ...
            \begin{tabularx}{\linewidth}{>{\RaggedRight\arraybackslash}p{2.3cm}>{\centering\arraybackslash}p{1.6cm}>{\RaggedRight\arraybackslash}p{3.0cm}>{\RaggedRight\arraybackslash}X>{\centering\arraybackslash}p{2.1cm}}
                ...
                \addlinespace[2pt]
                \textit{City of Davao v. First Meridian}~\cite{scp2022firstmeridian} & G.R. 240078 \newline (2022) & Davao City Ord.~158-05 §423 (Mandatory Payment Under Protest) & RA 7160 §195; LGU cannot require payment under protest for business taxes. & Contradiction \\
                \addlinespace[2pt]
                \textit{City of Davao v. GSIS}~\cite{scp2005davaovgsis} & G.R. 127383 \newline (2005) & Real Property Tax Levies on GSIS Government Properties & RA 7160 §193, §234; LGC withdrew tax exemptions of GOCCs; assessment valid. & Entailment \\
                \addlinespace[2pt]
