...
\subsubsection{Span Pairing}
    ...
    \begin{description}
        ...
        \item[Stage 1 Dense Bi-Encoder Candidate Pool.] To identify the optimal neural retriever for the coarse search stage, the proponents evaluate eight candidate open-weight bi-encoders representing the four inductive categories reviewed in Section~\ref{sec:ir_candidate_models}:
        \begin{enumerate}
    ...
    \subsubsection{Two-Step Model Screening Protocol}
    Evaluating all 28 models through exhaustive multi-epoch hyperparameter sweeps on local hardware would require prohibitive GPU compute. Therefore, the proponents implement a Two-Step Model Screening Protocol:
    \begin{description}
        \item[Step 1: Rapid Zero-Shot Screening.] In the initial screening step, all 28 candidate architectures from each of the six families are evaluated directly out-of-the-box on the held-out test dataset ($\mathcal{D}_{\text{test}}$, $N = 53$) without prior fine-tuning. This rapid test measures the intrinsic deontic sensitivity of each model and quantifies baseline affirmative bias---the known failure mode where unadapted language models default toward Entailment or Neutral when reading complex legal text~\cite{alshehri2026cross, tran2026enhancing}. Models that exhibit severe affirmative bias or unmanageable inference latency are filtered out before supervised training.
        
        \item[Step 2: Supervised Fine-Tuning Sweep.] Architectures that demonstrate viable pre-alignment, domain sensitivity, and operational feasibility advance to the second step. In this stage, candidate models undergo systematic supervised fine-tuning across the training split ($N_{\text{train}} = 245$) and validation split ($N_{\text{val}} = 52$) using a standardized hyperparameter grid. The validation split is then utilized to sweep the cost-sensitive decision threshold ($\tau^* \in [0.25, 0.50]$) to maximize the $F_2$-score for statutory conflict detection.
    \end{description}

    \subsubsection{Shortlisting Criteria and Candidate Ablation Design}
    Candidate admission from the 28-model zero-shot screening pool into the supervised fine-tuning shortlist is governed by three operational criteria:
    \begin{description}
        \item[Deontic Conflict Sensitivity.] The candidate must demonstrate non-trivial zero-shot conflict detection capabilities ($R_{\text{contra}} \ge 0.50$ or Macro $F_1 \ge 0.50$) or specialized legal domain pretraining. Models that collapse entirely into affirmative classifications are excluded to avoid spending training resources on architectures with fatal inductive bias.
        
        \item[Edge Latency and Memory Envelope.] The architecture must execute per-pair inference in under 60~milliseconds on GPU with an fp16 VRAM footprint strictly below 2~GB, ensuring compatibility with standard LGU workstations. Heavy sequence-to-sequence or massive cross-lingual models that exceed these bounds are disqualified.
        
        \item[Ablation Representativeness.] Following the Empirical Meritocracy paradigm (Path~1), admitted candidates must formally represent one of four planned ablation axes: (1) Parameter and Context Scale (comparing base against large variants under 8,192-token windows), (2) NLI Pre-Alignment Warm-Start (isolating cold masked language models against warm multi-task NLI checkpoints), (3) Legal Domain Pretraining (comparing general legal encoders against massive administrative regulatory pretraining), and (4) Compact Edge Baselines (establishing CPU latency boundaries for non-GPU municipal environments).
    \end{description}
