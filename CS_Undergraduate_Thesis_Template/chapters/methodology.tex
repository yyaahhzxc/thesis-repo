...
def extract_statutory_corpus(base_url, start_year=1900, end_year=2026):
    ...
    return statute_records
        ...
        \end{enumerate}
        Together, these two channels yielded an archive of exactly 1,664 local enactments spanning from 1950 to 2025. When combined with the 25,432 national statutes, the total statutory search space comprises \textbf{27,096 legal enactments} ($N_{\text{sections}} = 176,421$). Unlike national statutes available in clean digital text, the municipal archive consists of historical scans processed through vision-language model transcription followed by targeted text normalization, as detailed in Section~\ref{sec:municipal_corpus_analysis}.
        ...
        \begin{table}[!htbp]
            ...
            \begin{tabularx}{\linewidth}{>{\RaggedRight}p{4.2cm}ccc>{\RaggedRight}X}
                \toprule
                ...
                \midrule
                Total Enactments ($N$) & 25,432 & 1,664 & \textbf{27,096} & Comprehensive legislative space \\
                Total Section Provisions & 164,620 & 11,801 & \textbf{176,421} & Total searchable retrieval units \\
                Operative Provisions & 164,620 & 8,160 & \textbf{172,780} & Candidate substantive rules \\
                Procedural Boilerplate Sections & 0 & 3,641 (30.85\%) & \textbf{3,641} & Pruned non-regulatory text \\
                Historical Enactment Span & 1900--2026 (126 yrs) & 1950--2025 (75 yrs) & \textbf{1900--2026} & Longitudinal jurisprudence \\
                Median Enactment Characters & 10,807 & 5,644 & \textbf{10,480} & Document-level scale disparity \\
                Median Enactment Words & 1,512 & 880 & \textbf{1,472} & Local text conciseness \\
                Median Provision Tokens & 122.0 & 90.0 & \textbf{120.0} & Granular provision baseline \\
                512-Token Compliance Rate & 94.83\% & 92.72\% & \textbf{94.69\%} & Standard transformer compatibility \\
                Operative 512-Token Compliance & 94.83\% & 90.06\% & \textbf{94.60\%} & High operative window fit \\
                Normative Conflict Scope & Vertical Preemption & Horizontal Coherence & \textbf{Dual Scope} & Full legal conflict coverage \\
                \bottomrule
            \end{tabularx}
        \end{table}

        Figure~\ref{fig:dual_corpus_length_compliance} illustrates the structural comparisons between the national and municipal collections. Panel~(a) shows the document length disparity, where national enactments display a broader spread and frequently exceed one hundred thousand characters. In contrast, municipal ordinances remain compact, centering tightly around their median of 5,644 characters. Panel~(b) plots the cumulative token distribution across extracted provisions, confirming that both national sections (94.83\%) and local sections (92.72\%) adhere closely to the 512-token transformer sequence ceiling. Across all 176,421 combined provisions, exactly 94.69\% fit cleanly within standard sequence windows, validating natural section boundaries as the primary unit of retrieval.

        \begin{figure}[!htbp]
            \centering
            \includegraphics[width=0.96\linewidth]{figs/dual_corpus_length_compliance.png}
            \caption{Enactment Length Asymmetry (Full Text) and Provision-Level Cumulative 512-Token Compliance Curves across the Dual Statutory Search Space ($N = 27,096$ Enactments, $N_{\text{sections}} = 176,421$). Panel (a) illustrates document length distributions on a logarithmic scale, highlighting national omnibus codifications against concise municipal ordinances. Panel (b) depicts cumulative token compliance curves for national provisions (94.83\%), all municipal sections (92.72\%), operative substantive sections (90.06\%), and the 512-token transformer attention threshold.}
            \label{fig:dual_corpus_length_compliance}
        \end{figure}
