...
This section maps how data flows through the proposed system. The architecture follows a basic Input-Process-Output (IPO) logic, broken into a two-step ``Coarse-to-Fine'' pipeline. By separating the task of searching for documents from the task of checking their logic, the system avoids the hardware bottlenecks that usually cause large language models to fail. The steps below outline how a raw document is processed into a final conflict alert.
    ...
    \begin{description}
        \item[System Input.] The live workflow begins when a legislative researcher uploads a newly proposed ex-ante ordinance draft into the system. Because these proposed measures are newly prepared within local government offices, the input files are digital-born documents exported directly from word processors or digital PDF formats, avoiding optical character recognition errors. The system automatically segments the submitted document at section boundaries, isolating operative substantive mandates while filtering out routine procedural boilerplate.
        ...
        \item[Computational Process.] This part involves two main stages: information retrieval to find relevant laws and textual entailment to perform conflict detection.
            \begin{description}
                \item[Stage 1: Information Retrieval.] The system first acts as a search engine to quickly narrow down the large database. It combines a keyword search (BM25) adapted from Team AIIR~\cite{wu2025aiir} with a dense retrieval model (Bi-Encoder) inspired by Team JNLP~\cite{coliee2025proceedings}. While the initial architectural design was guided by the COLIEE 2025 proceedings, the subsequent release of the COLIEE 2026 competition benchmarks~\cite{coliee2026proceedings, ngo2026nowj} reinforced this hybrid approach as optimal for statutory candidate recall. The keyword search identifies exact statutory citations and numerical penalty amounts, while the Bi-Encoder measures conceptual similarity in vector space. Candidate positions from both retrievers are merged through reciprocal rank fusion and hierarchy safeguards to extract only the top-$k$ most relevant statutory candidates~\cite{cormack2009reciprocal}. This hybrid candidate pool provides a focused group of provisions for the logic check without overburdening local computer memory.

                \item[Stage 2: Natural Language Inference.] The pipeline then moves to logical reasoning using the selected NLI model, following the framework developed by Team KIS~\cite{kadowaki2025modernbert}. In natural language processing, Natural Language Inference (NLI)—historically termed Recognizing Textual Entailment (RTE)—is the foundational task of determining the directional logical relationship between two statements: whether a hypothesis is entailed (logically necessitated), contradicted (logically impossible), or neutral (logically independent) given an established premise~\cite{dagan2006pascal, bowman2015snli}. In this system, the retrieved statutory provision serves as the premise and the proposed draft ordinance serves as the hypothesis, framing statutory preemption detection as an automated contradiction classification problem. The system joins the proposed draft provisions and the retrieved laws into a single input sequence. Following the classification strategy of Team KIS~\cite{kadowaki2025modernbert}, the lightweight encoder skips text generation and performs direct classification. It evaluates the combined sequence using separation tokens as classification points, producing a probability score for each provision to determine if a conflict exists.
            \end{description}

        \item[System Output.] In the final step, the system delivers structured diagnostic findings based on calibrated conflict probabilities. If no normative collisions are identified, the system confirms legislative compliance. When an inconsistency is detected, the system categorizes the finding into a two-tiered alert structure: a Critical Vertical Preemption Alert for collisions with national statutes under the \textit{Magtajas} doctrine, or an Informational Horizontal Inconsistency Notice for modifications of prior municipal ordinances. To keep the process transparent, the dashboard provides intrinsic structural explainability by displaying side-by-side comparative spans, calibrated confidence percentages, and highlighted conflicting phrases.
    \end{description}
...
\subsection{Research Design}
    ...
    Because the raw downloaded files vary widely in formatting, they require thorough cleaning, grouping, and text normalization. While national statutes are already available as clean digital text, the scanned Davao City ordinances contain most of the OCR errors in the dataset. Since these local records come from image-based PDFs, extracting the text often produces misread characters and broken lines. The pipeline uses rule-based filters to find and correct these errors specifically within the local ordinance files. This targeted cleaning ensures that the subsequent vector conversion process is not disrupted by broken words or garbled text.
        ...
        \begin{equation}
            ...
            \label{eq:c_tfidf}
        \end{equation}
        \eqcaption{Class-based Term Frequency-Inverse Document Frequency ($c\text{-}TF\text{-}IDF$)}
        ...
        \end{equation}
        \eqcaption{Macro Legal Domain Centroid and Softmax Probability}
        ...
        \begin{equation}
            ...
            \label{eq:max_pooled_subchunk}
        \end{equation}
        \eqcaption{Max-Pooled Sub-Chunk Contradiction Probability}
        ...
        \end{equation}
        \eqcaption{Hierarchical Metadata-Prepended Statutory Chunk Representation}
...
\subsubsection{Span Pairing}
    ...
    \begin{description}
        ...
        \item[Stage 2 Inference Baseline (Zero-Shot Cross-Encoder).] For fine natural language inference, the pipeline establishes a baseline using a standard pretrained, zero-shot Cross-Encoder architecture without domain adaptation, following the foundational legal textual entailment baseline established by Rabelo et al.~\cite{rabelo2022coliee} for COLIEE Task~4. While general-domain language models exhibit broad linguistic competence, legal text requires distinguishing subtle statutory carve-outs from unlawful overreach. Selecting an unadapted zero-shot Cross-Encoder baseline directly tests the hypothesis that specialized few-shot fine-tuning on expert-annotated Philippine statutory pairs is necessary to overcome affirmative bias and achieve reliable contradiction detection.
    \end{description}
    ...
    Before deploying the system, the researchers evaluate candidate Cross-Encoder models to find the most accurate architecture. Candidate models are tested against the curated Ground Truth dataset. To fit local hardware limits, the candidate pool is restricted to lightweight and memory-efficient models. Specifically, the evaluation compares standard-context architectures, such as MiniLM~\cite{reimers2019sentencebert} (22.7M parameters) and DeBERTa-v3~\cite{he2023debertav3} (86M backbone parameters), against the native long-context architecture ModernBERT~\cite{warner2024modernbert, kadowaki2025modernbert} (149M parameters). While leading systems in the COLIEE 2026 competition explored large generative ensembles requiring substantial GPU clusters~\cite{alshehri2026cross, ngo2026nowj}, the proponents intentionally evaluate compact, encoder-only models to satisfy the computing limits of local government workstations. The researchers hypothesize that while ModernBERT guarantees complete sequence coverage on lengthy statutory provisions up to 8,192 tokens, standard-context architectures paired with sub-chunking may provide superior contradiction precision and lower latency across the 94.83\% majority of concise provisions. Once the empirical grid search is completed and the optimal architecture is selected based on validation Macro $F_1$, its exact model weights and tokenizer configurations will be permanently pinned and packaged for offline execution (\texttt{local\_files\_only = True}), ensuring that the production pipeline runs within air-gapped municipal IT networks without external dependencies. Testing these competing architectures against a standardized, expert-reviewed benchmark reveals which model best balances context capacity with logical reasoning accuracy.
    ...
    Evaluating the two-stage pipeline requires separate metrics for each operational phase. The coarse retrieval phase in Stage~1 prioritizes candidate recall over final classification, while the inference phase in Stage~2 requires high precision to avoid false conflict alerts.
        ...
        \end{equation}
        \eqcaption{Information Retrieval Recall at Rank $k$ ($\text{Recall@}k$)}
        ...
        \begin{equation}
            ...
            \label{eq:mrr_at_k}
        \end{equation}
        \eqcaption{Mean Reciprocal Rank at Rank $k$ ($\text{MRR@}k$)}
        ...
        \begin{equation}
            ...
            \label{eq:class_metrics}
        \end{equation}
        \eqcaption{Class-Specific Precision, Recall, and $F_1$-Score}
        ...
        \begin{equation}
            ...
            \label{eq:macro_f1}
        \end{equation}
        \eqcaption{Macro-Averaged $F_1$-Score Across Logical Classes}
        ...
        \begin{equation}
            ...
            \label{eq:f2_score}
        \end{equation}
        \eqcaption{Cost-Sensitive Contradiction $F_2$-Score}
        \begin{equation}
            \tau^* = \arg\max_{\tau \in [0.25, 0.50]} F_{2, \text{conflict}}(\tau; \mathcal{D}_{\text{val}})
            \label{eq:threshold_tuning}
        \end{equation}
        \eqcaption{Optimal Contradiction Decision Threshold Calibration ($\tau^*$)}
        ...
        \begin{equation}
            ...
            \label{eq:max_pool_conflict}
        \end{equation}
        \eqcaption{Section-Level Max-Pooled Contradiction Probability}
        A section triggers an operative conflict warning if and only if $P(\text{Conflict} \mid S_i) \ge \tau^*$, with the system highlighting the specific governing candidate provision $P_j^* = \arg\max_{j} P(\text{Contradiction} \mid P_j, S_i)$ that produced the highest conflict probability. This max-pooling aggregation ensures that an authentic statutory clash ranked lower in the candidate shortlist is not obscured by neutral or harmonious provisions appearing at higher retrieval ranks.
        ...
        \begin{description}
            ...
            \begin{equation}
                ...
                \label{eq:mcnemar_test}
            \end{equation}
            \eqcaption{McNemar's Chi-Squared Test with Edwards' Continuity Correction}
            ...
            \begin{equation}
                ...
                \label{eq:friedman_test}
            \end{equation}
            \eqcaption{Friedman Two-Way Analysis of Variance by Ranks ($\chi_F^2$)}
    ...
    \begin{description}
        ...
        \begin{equation}
            ...
            \label{eq:cascaded_hit}
        \end{equation}
        \eqcaption{Cascaded End-to-End Joint Conflict Detection Hit Indicator}
        ...
        \item[Tier 2: Injected-Fault Full-Document Stress Testing.] The second evaluation tier tests how the complete pipeline processes full, multi-section legislative drafts rather than isolated sentence pairs. In municipal practice, proposed ordinances contain extensive procedural text, including administrative definitions and standard effectivity clauses, which should not trigger conflict warnings. Adapting the behavioral perturbation and fault-injection principles formulated by Ribeiro et al.~\cite{ribeiro2020beyond} and Koreeda and Manning~\cite{koreeda2021contract}, the proponents construct full synthetic ordinance drafts containing controlled legal faults. In each synthetic document, 80\% to 90\% of the sections remain compliant with national law, while one or two targeted sections contain deliberate statutory violations, such as excessive penalty amounts or unlicensed commercial permissions. The automated preprocessor chunks the submitted document into operative sections, auditing every section independently against the statutory corpus.
        ...
        \begin{equation}
            ...
            \label{eq:section_specificity}
        \end{equation}
        \eqcaption{Section-Level Specificity for Full-Document Stress Testing}
        ...
        \item[Tier 3: Historical Jurisprudential Validation and Human Expert Evaluation.] The third evaluation tier validates the practical utility and legal soundness of the system using documented historical legal disputes and active expert review. Rather than relying solely on synthetic tests, the proponents evaluate the pipeline against historical municipal ordinances that were formally contested in Philippine courts. Specifically, the system audits historical draft records from landmark preemption rulings, including Davao City Ordinance No.~0309-07, which was invalidated by the Supreme Court in \textit{Mosqueda v. Pilipino Banana Growers \& Exporters Association, Inc.}~\cite{scp2016mosqueda} for conflicting with national administrative authority under the \textit{Magtajas} doctrine~\cite{scp1994magtajasvpryce}. By comparing automated alerts against published court decisions and mayoral veto messages from the Davao City archives, the evaluation verifies whether the pipeline surfaces the exact statutory provisions cited by judicial authorities. This historical validation proves that the system identifies authentic statutory preemption points that previously led to municipal litigation.

        To measure user acceptance in daily legislative workflows, the system is deployed to the 15 active Sangguniang Panlungsod legal researchers. Evaluators audit sample draft ordinances using the single-page prototype dashboard during First Reading committee preparation. The researchers assess whether the retrieved statutory context and highlighted attention heatmaps clarify potential preemption risks under Section~458 of Republic Act No.~7160~\cite{philgov1991ra7160}. Human agreement across evaluator judgments is measured using Fleiss' Kappa ($\kappa$)~\cite{fleiss1971measuring}, while subjective software usability is evaluated using the standardized System Usability Scale (SUS)~\cite{brooke1996sus}. Combining historical court rulings with direct feedback from city council staff confirms that the finished pipeline provides practical decision support for local governance.
    \end{description}
    ...
    \begin{enumerate}
        \item \textbf{Commit Hash Pinning:} All empirical tables, benchmark figures, and model checkpoints reported in this thesis are permanently tagged with their respective Git commit hashes (e.g., Commit \texttt{cc1c33d} for Milestone~1, Commit \texttt{4dd7c85} for legal instruments), ensuring complete version traceability.
