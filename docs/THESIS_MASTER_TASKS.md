...
- [x] **Migrate Experiments from Chapter 3 to Chapter 4**: Relocated Table 4.1 (Full-Corpus BM25 Benchmark) and Table 4.2 (9-Model Candidate Ablation Benchmark) along with Figure 4.1 from Chapter 3 to Chapter 4, while retaining Exploratory Data Analysis (EDA) and corpus distributions in Chapter 3. `(Yah)`
...
- [x] **Search Candidate Models for Stage 2**: Conducted comprehensive literature and benchmark research across Hugging Face NLI leaderboards, LexGLUE, COLIEE Task 4, and MTEB; compiled a 28-model long list and finalized the Top 10 recommended candidate models (categorized across Legal Domain, NLI Pre-Alignment, Long-Context, Native Rerankers, Multilingual, and Edge Efficiency) to complement MiniLM, DeBERTa-v3, and ModernBERT (documented in [`docs/STAGE2_CANDIDATE_MODELS_RESEARCH.md`](file:///c:/Users/HP/Documents/GitHub/thesis-repo/docs/STAGE2_CANDIDATE_MODELS_RESEARCH.md)). `(Yah)`
