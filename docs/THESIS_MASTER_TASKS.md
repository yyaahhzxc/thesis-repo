...
- [ ] **In-Paper Algorithms & Pseudocode**: Insert formal algorithmic pseudocode environments in Chapter 3 for Stage 1 (Dual-Stream BM25 + Bi-Encoder fused via RRF $k=60$) and Stage 2 (Cross-Encoder NLI with cost-sensitive threshold $\tau^*$). `(Yah)`
- [ ] **Corpus & Sourcing Visual Figures**:
  - Screenshot of the Lawphil Project web interface showing statutory indices and HTML structure.
  - Formatted JSONL code listing showing an extracted national statute record with metadata.
  - High-resolution scan of an original Davao City local ordinance PDF showing scanning artifacts, city seal, and official signatures. `(Yah / Ralph)`
